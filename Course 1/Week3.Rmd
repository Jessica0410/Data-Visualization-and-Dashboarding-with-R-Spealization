...
```{r,problem4}
boxplot(dat$faminc_new, main="YuChen's Boxplot of Family Income", xlab ="Family Income")
```
...
dat1<-dat|>filter(gender==1&region %in% 1:2&marstat==1&newsint==1)
str(dat1)
```
...
##CC18_325d supports increasing the standard deduction from 12,000 to 25,000
##CC18_325a supports cutting the corporate income tax rate from 39 to 21 percent

c325a<- dat$CC18_325a |>
  recode('1'=1,'2'=0)
c325d<- dat$CC18_325d |>
  recode('1'=1,'2'=0)
tax_scale <- c325a + c325d
...
```{r problem10}
dat|>
  filter(investor==2,between(faminc_new,5,10))|>
  summarize(
...
qplot(data = dat,x = pid7,geom = "histogram", xlab = "Seven Point Party ID", ylab = "Count")
```
